...
parseLib :: forall a. Schema -> String -> Lib (StrMap a)
...
parseGroup schema lib group = do
  ...
  obj <- if (not $ null parent) then
           case (lookup parent lib) of
             (Just p) -> return p
             Nothing  -> Left $ LibError $ "couldn't find parent " ++ parent
         else
      ...
      case (stripPrefix "--" line) of
        (Just str) -> do
          ...
          case cmp of
            ...
            _      -> Left $ LibError "wtf are you doing"
        Nothing -> Left $ LibError $ "expecting a name: " ++ line
    parseName Nothing = Left $ LibError "group doesnt have title line"
